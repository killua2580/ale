    ...
    public partial class App : Application
    {
        ...
        private void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSingleton<IBookService>(provider => 
                new IHECLibrary.Services.Implementations.SupabaseBookService(provider.GetRequiredService<IUserService>(), supabaseClient));
            ...
            services.AddSingleton<MainWindowViewModel>(provider => new MainWindowViewModel(
                provider.GetRequiredService<INavigationService>(),
                provider.GetRequiredService<INavigationService>().GetInitialViewModel()
            ));            // Add WelcomeViewModel registration
            services.AddTransient<WelcomeViewModel>(provider => new WelcomeViewModel(
                provider.GetRequiredService<INavigationService>()
            ));

            // Add WhoAmIViewModel registration
            services.AddTransient<WhoAmIViewModel>(provider => new WhoAmIViewModel(
                provider.GetRequiredService<INavigationService>()
            ));
        }
    }
